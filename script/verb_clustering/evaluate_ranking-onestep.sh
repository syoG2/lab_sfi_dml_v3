...
for setting in ${settings[@]}; do
    for model_name in ${model_names[@]}; do
        for ranking_method in ${ranking_methods[@]}; do
            ...
            d3=${vec_type}/${clustering_name}-${clustering_method}
            d4=${vec_type}-${clustering_name}-${clustering_method}
            d5=${ranking_method}
            python ${source_dir}/evaluate_ranking.py \
                ...
                --output_dir ${data_dir}/ranking/${d1}/${d2}/${d4}/${d5} \
                --ranking_method ${ranking_method} \
                --split ${split}

            python ${source_dir}/evaluate_ranking_overlap.py \
                --input_score_file ${data_dir}/ranking/${d1}/${d2}/${d4}/${d5}/score_${split}.jsonl \
                --input_train_file ${data_dir}/dataset/${d1}/exemplars_train.jsonl \
                --output_dir ${data_dir}/ranking_overlap/${d1}/${d2}/${d4}/${d5} \
                --split ${split}
        done
    done
done
