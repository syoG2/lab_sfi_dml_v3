...
for model_name in "${model_names[@]}"; do
    for setting in "${settings[@]}"; do
        for vec_type in "${vec_types[@]}"; do
            ...
            # [x]:C4でクラスタリングしたものにFrameNetの用例を割り当てるスクリプトを作成
            uv run python ${source_dir}/nearest_neighbor.py \
                --input_embedding_dir "${data_dir}/embedding/${verb_form}/${add_method}/${add_key}/${c4_rate}/${d1}" \
                --input_clustering_dir "${data_dir}/clustering/${verb_form}/${add_method}/${add_key}/${clustering_dataset}/${c4_rate}/${d1}/${d2}" \
                --output_dir "${data_dir}/clustering/${verb_form}/${add_method}//${add_key}/${clustering_dataset}/${c4_rate}/${d1}/${d2}" \
                --input_params_file "${data_dir}/best_params_clustering/${verb_form}/${add_method}/${add_key}/${clustering_dataset}/${c4_rate}/${d1}/${d2}/best_params.json"
