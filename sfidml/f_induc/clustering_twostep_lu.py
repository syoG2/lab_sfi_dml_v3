...
class TwostepClusteringLU:
    ...
    def _make_vec_1st(self, df, vec_array, lu_name):
        df_lu_name = df[df["lu_name"] == lu_name].copy()
        lu_name_vec_array = vec_array[df_lu_name["vec_id"]]
        return df_lu_name, lu_name_vec_array
    ...
    def make_confusion_matrix(self, df):
        true = df.groupby("lu_name").nunique()["frame"].values
        pred = df.groupby("lu_name").max()["plu_local"].values
    ...
    def make_params(self, df, vec_array):
        ...
        if self.clustering_method1 == "average":
            lth_list = []
            for lu_name in sorted(set(df["lu_name"])):
                df_lu_name = df[df["lu_name"] == lu_name]
                lu_name_vec_array = vec_array[df_lu_name["vec_id"]]
                lth_dict = {
                    "lu_name": lu_name,
                    "n_frames": len(set(df_lu_name["frame"])),
                    "n_texts": len(lu_name_vec_array),
                }
                if len(lu_name_vec_array) >= 2:
                    z = linkage(
                        pdist(lu_name_vec_array),
            ...
            # )
            params["kmax"] = max(
                df.groupby("lu_name")["frame"].agg(set).apply(lambda x: len(x))
            )
        ...
        vf2f = {
            vf: f
            for vf, f in zip(
                df.apply(lambda row: row["lu_name"] + "_" + row["frame"], axis=1),
                df["frame"],
            )
        }
        params["gth"] = sum(
    ...
    def step(self, df, vec_array1, vec_array2, params):
        ...
        for lu_name in sorted(set(df["lu_name"])):
            df_1st, vec_array_1st = self._make_vec_1st(df, vec_array1, lu_name)
