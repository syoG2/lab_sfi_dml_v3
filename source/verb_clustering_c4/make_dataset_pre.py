            ...
            for part_id in tqdm(part_id_list, leave=False):
                for split_name in tqdm(split_name_list, leave=False):
                    ...
                    if input_file.exists():
                            ...
                            if args.add_data == "lu":
                                ...
                                for n in tqdm(range(args.n_splits), leave=False):
                                    ...
                                    verb_list[n] = [
                                        ...
                                        if verb not in verb_to_remove
                                    ]
                        elif args.add_method == "frequency":
                            if args.add_data == "lu":
                                for n in tqdm(range(args.n_splits), leave=False):
                                    setting = settings[n]
                                    additional = df_c4[
                                    ...
                                    # c4のデータが十分集まったLUはこれ以降追加しない
                                    lu_to_remove = counts[
                                        args.frequency_max <= counts["c4_count"]
                                    ].index.tolist()
                        ...
                        if (len(lu_name_list) == 0) | (len(verb_list) == 0):
                            break
                    else:
                        tqdm.write(f"{input_file} is not found.")
                        # ここまで
        ...
        if args.add_method == "ratio":
            if args.add_data == "lu":
                ...
                for n in tqdm(range(args.n_splits)):
                    ...
                    with open(output_dir / "incomplete_verbs.txt", "w") as f:
            ...
            if args.add_data == "lu":
                ...
                for n in tqdm(range(args.n_splits)):
                    setting = settings[n]
                    framenet_count = df[
                        (df["source"] == "framenet") & (df[setting] == "test")
                    ].shape[0]
                    c4_count = df[
                        (df["source"] == "c4") & (df[setting] == "test")
                    ].shape[0]
                    if c4_count > framenet_count * args.c4_rate:
                        remove = df[
                            (df["source"] == "c4") & (df[setting] == "test")
                ...
                for n in tqdm(range(args.n_splits)):
                    ...
                    output_dir.mkdir(parents=True, exist_ok=True)
                    with open(output_dir / "incomplete_lus.txt", "w") as f:
                        for lu in incomplete_verbs[n]:
                            print(lu, file=f)

    for n in tqdm(range(args.n_splits)):
        setting = f"{args.setting_prefix}_{args.n_splits}_{n}"
        if args.add_method == "frequency":
            output_dir = (
                args.output_dir / str(args.c4_rate) / str(args.frequency_max) / setting
            )
        else:
            output_dir = args.output_dir / str(args.c4_rate) / setting
        output_dir.mkdir(parents=True, exist_ok=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--add_data", type=str, choices=["lu", "verb"])
    # parser.add_argument(
    #     "--add_method",
    #     type=str,
    #     choices=[
